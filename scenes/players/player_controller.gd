...
@export var actions : PlayerInputResource
...
@export_range(50, 500) var maxSpeed: float = 175.0
...
##If enabled, the default movement speed will by 1/2 of the maxSpeed and the player must hold a actions.run button to accelerate to max speed. Assign actions.run (case sensitive) in the project input settings.
@export var runningModifier: bool = false

#INFO JUMPING 
@export_category("Jumping and Gravity")
##The peak height of your player's jump
@export_range(0, 20) var jumpHeight: float = 2.1
...
@export_range(0, 100) var gravityScale: float = 23.0
##The fastest your player can fall
@export_range(0, 1000) var terminalVelocity: float = 700.0
##Your player will move this amount faster when falling providing a less floaty jump curve.
@export_range(0.5, 3) var descendingGravityFactor: float = 2
##Enabling this toggle makes it so that when the player releases the jump key while still ascending, their vertical Player.velocity will cut by the height cut, providing variable jump height.
@export var shortHopAkaVariableJumpHeight: bool = true
##How much the jump height is cut by.
@export_range(1, 10) var jumpVariable: float = 8
...
##wall latching must be enabled for this to work. #If enabled, the player must hold down the actions.latch key to wall latch. Assign actions.latch in the project input settings. The player's input will be ignored when latching.
...
@export_range(1, 5) var correctionAmount: float = 1
...
##Holding down and pressing the input for actions.roll will execute a roll if the player is grounded. Assign a actions.roll input in project settings input.
...
##Animations must be named actions.run all lowercase as the check box says
@export var run: bool
##Animations must be named actions.jump all lowercase as the check box says
...
##Animations must be named actions.latch all lowercase as the check box says
...
##Animations must be named actions.roll all lowercase as the check box says
...
func _process(_delta):
	#INFO animations
	#directions
	if Player.is_on_wall() and !Player.is_on_floor() and latch and wallLatching and ((wallLatchingModifer and latchHold) or !wallLatchingModifer):
		latched = true
	else:
		latched = false
		wasLatched = true
	...
	if run and idle and !dashing and !crouching and !walk:
		if abs(Player.velocity.x) > 0.1 and Player.is_on_floor() and !Player.is_on_wall():
			anim.speed_scale = abs(Player.velocity.x / 150)
			anim.play(actions.run)
		elif abs(Player.velocity.x) < 0.1 and Player.is_on_floor():
			anim.speed_scale = 1
			anim.play("idle")
	elif run and idle and walk and !dashing and !crouching:
		if abs(Player.velocity.x) > 0.1 and Player.is_on_floor() and !Player.is_on_wall():
			anim.speed_scale = abs(Player.velocity.x / 150)
			if abs(Player.velocity.x) < (maxSpeedLock):
				anim.play("walk")
			else:
				anim.play(actions.run)
		elif abs(Player.velocity.x) < 0.1 and Player.is_on_floor():
	...
	#jump
	if Player.velocity.y < 0 and jump and !dashing:
		anim.speed_scale = 1
		anim.play(actions.jump)
		
	if Player.velocity.y > 40 and falling and !dashing and !crouching:
		anim.speed_scale = 1
		anim.play("falling")
	...
	if latch and slide:
		...
		if latched and !wasLatched:
			anim.speed_scale = 1
			anim.play(actions.latch)
		if Player.is_on_wall() and Player.velocity.y > 0 and slide and anim.animation != "slide" and wallSliding != 1:
			anim.speed_scale = 1
			anim.play("slide")
...
func _physics_process(delta):
	...
	leftHold = Input.is_action_pressed(actions.left)
	rightHold = Input.is_action_pressed(actions.right)
	upHold = Input.is_action_pressed(actions.up)
	downHold = Input.is_action_pressed(actions.down)
	leftTap = Input.is_action_just_pressed(actions.left)
	rightTap = Input.is_action_just_pressed(actions.right)
	leftRelease = Input.is_action_just_released(actions.left)
	rightRelease = Input.is_action_just_released(actions.right)
	jumpTap = Input.is_action_just_pressed(actions.jump)
	jumpRelease = Input.is_action_just_released(actions.jump)
	runHold = Input.is_action_pressed(actions.run)
	latchHold = Input.is_action_pressed(actions.latch)
	dashTap = Input.is_action_just_pressed(actions.dash)
	rollTap = Input.is_action_just_pressed(actions.roll)
	downTap = Input.is_action_just_pressed(actions.down)
	twirlTap = Input.is_action_just_pressed(actions.twirl)
	...
	if rightHold and leftHold and movementInputMonitoring:
		...
		if Player.velocity.x > maxSpeed or instantAccel:
	...
	#INFO Crouching
	if crouch:
		if downHold and Player.is_on_floor():
			crouching = true
	...
	#INFO Rolling
	if canRoll and Player.is_on_floor() and rollTap and crouching:
		_rollingTime(rollLength * 0.25)
		if wasPressingR and !(upHold):
			Player.velocity.y = 0
			Player.velocity.x = maxSpeedLock * rollLength
			dashCount += -1
	...
	if Player.is_on_wall() and !groundPounding:
		...
		elif wallSliding != 1 and Player.velocity.y > 0:
			appliedGravity = appliedGravity / wallSliding
	elif !Player.is_on_wall() and !groundPounding:
		appliedTerminalVelocity = terminalVelocity
	
	if gravityActive:
		if Player.velocity.y < appliedTerminalVelocity:
			Player.velocity.y += appliedGravity
		elif Player.velocity.y > appliedTerminalVelocity:
				Player.velocity.y = appliedTerminalVelocity
		
	if shortHopAkaVariableJumpHeight and jumpRelease and Player.velocity.y < 0:
		Player.velocity.y = Player.velocity.y / jumpVariable
	...
	if jumps == 1:
		...
		if jumpTap and !Player.is_on_wall():
			if coyoteActive:
				coyoteActive = false
				_jump()
			if jumpBuffering > 0:
				jumpWasPressed = true
				_bufferJump()
			elif jumpBuffering == 0 and coyoteTime == 0 and Player.is_on_floor():
				_jump()
		elif jumpTap and Player.is_on_wall() and !Player.is_on_floor():
			if wallJump and !latched:
				_wallJump()
			elif wallJump and latched:
				_wallJump()
		elif jumpTap and Player.is_on_floor():
			_jump()
		...
		if Player.is_on_floor():
			jumpCount = jumps
		if jumpTap and Player.is_on_wall() and wallJump:
			_wallJump()
	...
	if twoWayDashVertical and dashTap and dashCount > 0 and !rolling:
		...
		if upHold and downHold:
			...
			_pauseGravity(dTime)
			Player.velocity.x = 0
			Player.velocity.y = -dashMagnitude
			dashCount += -1
			movementInputMonitoring = Vector2(false, false)
			_inputPauseReset(dTime)
	...
	if dashing and Player.velocity.x > 0 and leftTap and dashCancel:
		Player.velocity.x = 0
	if dashing and Player.velocity.x < 0 and rightTap and dashCancel:
		Player.velocity.x = 0
	
	#INFO Corner Cutting
	if cornerCutting:
		if Player.velocity.y < 0 and leftRaycast.is_colliding() and !rightRaycast.is_colliding() and !middleRaycast.is_colliding():
			Player.position.x += correctionAmount
		if Player.velocity.y < 0 and !leftRaycast.is_colliding() and rightRaycast.is_colliding() and !middleRaycast.is_colliding():
			Player.position.x -= correctionAmount
			
	#INFO Ground Pound
	if groundPound and downTap and !Player.is_on_floor() and !Player.is_on_wall():
		groundPounding = true
		gravityActive = false
		Player.velocity.y = 0
		await get_tree().create_timer(groundPoundPause).timeout
		_groundPound()
	if Player.is_on_floor() and groundPounding:
		_endGroundPound()
...
func _wallJump():
	...
	Player.velocity.y = -verticalWallKick
	var dir = 1
	if wallLatchingModifer and latchHold:
		dir = -1
	if wasMovingR:
		Player.velocity.x = -horizontalWallKick  * dir
	else:
		Player.velocity.x = horizontalWallKick * dir
...
func _dashingTime(time):
	...
	dashing = false
	if !Player.is_on_floor():
		Player.velocity.y = -gravityScale * 10
...
func _groundPound():
	...
	Player.velocity.y = jumpMagnitude * 2
